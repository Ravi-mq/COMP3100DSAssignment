...
public class Stage2Client {
    ...
    public static void main(String args[]) {
        Socket socket = null;
        try { // using set of try catch for exception handling such as error encountered
            ...
            while (!serverReply.equals("NONE")) { // if server got one or more than one job.
                ...
                // finding first 'available' server and if not then 'capable' server
                if (serverReply.contains("JOBN")) {
                    ...
                    try {
                        ...
                        if (Integer.parseInt(recordsArr[1]) != 0) { // if no server readily available
                        ...
                        } else { // if no readily available server available then use the capable servers
            ...
            /*-------------------------------Terminating the connection if no available jobs-------------------------------------------------*/
            clientMsg = "QUIT\n";
            dataOut.write(clientMsg.getBytes());
            dataOut.flush();
            serverReply = dataIn.readLine();
